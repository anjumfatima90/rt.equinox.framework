...
public class ResolverImpl implements Resolver {
	...
	public Map<Resource, List<Wire>> resolve(ResolveContext rc) throws ResolutionException {
		if (m_executor != null) {
			...
			final ExecutorService executor = System.getSecurityManager() != null ? AccessController.doPrivileged(new PrivilegedAction<ExecutorService>() {
				public ExecutorService run() {
					return Executors.newFixedThreadPool(m_parallelism);
				}
			}, m_acc) : Executors.newFixedThreadPool(m_parallelism);
			try {
			...
			try {
				...
				// resolve, so populate the wire map.
					...
					if (session.isDynamic()) {
						wireMap = populateDynamicWireMap(session.getContext(), session.getDynamicHost(), session.getDynamicRequirement(), wireMap, allCandidates);
					} else {
						for (Resource resource : allCandidates.getRootHosts().keySet()) {
							if (allCandidates.isPopulated(resource)) {
								wireMap = populateWireMap(session.getContext(), allCandidates.getWrappedHost(resource), wireMap, allCandidates);
							}
						}
					}
				}
	...
	private Candidates findValidCandidates(ResolveSession session, Map<Resource, ResolutionError> faultyResources) {
		...
		do {
			...
			session.setCurrentError(checkConsistency(session, allCandidates, currentFaultyResources));
	...
	private ResolutionError checkConsistency(ResolveSession session, Candidates allCandidates, Map<Resource, ResolutionError> currentFaultyResources) {
		...
		for (Entry<Resource, Resource> entry : allhosts.entrySet()) {
			rethrow = checkPackageSpaceConsistency(session, entry.getValue(), allCandidates, session.isDynamic(), resourcePkgMap, resultCache);
			if (rethrow != null) {
	...
	private static Packages getPackages(ResolveSession session, Candidates allCandidates, Map<Resource, List<WireCandidate>> allWireCandidates, Map<Resource, Packages> allPackages, Resource resource, Packages resourcePkgs) {
		...
		for (WireCandidate wire : allWireCandidates.get(resource)) {
			...
			if (Util.isDynamic(wire.requirement)) {
				...
				if (resourcePkgs.m_exportedPkgs.containsKey(pkgName) || resourcePkgs.m_importedPkgs.containsKey(pkgName) || resourcePkgs.m_requiredPkgs.containsKey(pkgName)) {
			...
			mergeCandidatePackages(session, allPackages, allCandidates, resourcePkgs, wire.requirement, wire.capability, new HashSet<Capability>(), new HashSet<Resource>());
		}

		return resourcePkgs;
	}

	private static void computeUses(ResolveSession session, Map<Resource, List<WireCandidate>> allWireCandidates, Map<Resource, Packages> resourcePkgMap, Resource resource) {
		...
		if ((wiring == null) || isDynamicImporting) {
			...
			for (WireCandidate w : wireCandidates) {
				...
				// considered below.
				if (!req.getNamespace().equals(BundleNamespace.BUNDLE_NAMESPACE) && !req.getNamespace().equals(PackageNamespace.PACKAGE_NAMESPACE)) {
			...
			for (List<Blame> blames : resourcePkgs.m_importedPkgs.values()) {
				for (Blame blame : blames) {
					...
					mergeUses(session, resource, resourcePkgs, blame.m_cap, blameReqs, null, resourcePkgMap, usesCycleMap);
				}
			}
				...
				for (Wire w : candWiring.getRequiredResourceWires(null)) {
					if (w.getRequirement().getNamespace().equals(BundleNamespace.BUNDLE_NAMESPACE)) {
						if (Util.isReexport(w.getRequirement())) {
							mergeCandidatePackages(session, resourcePkgMap, allCandidates, packages, currentReq, w.getCapability(), capabilityCycles, visitedRequiredBundles);
						}
					}
				}
			} else {
				for (Requirement req : candCap.getResource().getRequirements(null)) {
					if (req.getNamespace().equals(BundleNamespace.BUNDLE_NAMESPACE)) {
						if (Util.isReexport(req)) {
							...
							if (cap != null) {
								mergeCandidatePackages(session, resourcePkgMap, allCandidates, packages, currentReq, cap, capabilityCycles, visitedRequiredBundles);
							}
						}
					}
				}
	...
	private static void mergeUses(ResolveSession session, Resource current, Packages currentPkgs, Capability mergeCap, List<Requirement> blameReqs, Capability matchingCap, Map<Resource, Packages> resourcePkgMap, Set<Capability> cycleMap) {
			...
			for (String usedPkgName : uses) {
				...
				for (Blame blame : candSourceBlames) {
					if (blame.m_reqs != null) {
						...
						mergeUses(session, current, currentPkgs, blame.m_cap, blameReqs2, matchingCap, resourcePkgMap, cycleMap);
					} else {
						addUsedBlame(usedPkgBlames, blame.m_cap, blameReqs, matchingCap);
						mergeUses(session, current, currentPkgs, blame.m_cap, blameReqs, matchingCap, resourcePkgMap, cycleMap);
					}
				}
			}
		}
	...
	private static Map<Resource, Packages> calculatePackageSpaces(final ResolveSession session, final Candidates allCandidates, Collection<Resource> hosts) {
		...
		for (final Resource resource : allWireCandidates.keySet()) {
			...
			executor.execute(new Runnable() {
				public void run() {
					calculateExportedPackages(session, allCandidates, resource, packages.m_exportedPkgs, packages.m_substitePkgs);
				}
			});
		}
		executor.await();
	...
	private static void addUsedBlame(ArrayMap<Capability, UsedBlames> usedBlames, Capability usedCap, List<Requirement> blameReqs, Capability matchingCap) {
			...
			if (blames.size() > 1) {
				...
				for (Blame blame : blames) {
					if (sourceBlame == null) {
						...
						// Report conflict.
						rethrow = new UseConstraintError(session.getContext(), allCandidates, resource, pkgName, sourceBlame, blame);
						if (m_logger.isDebugEnabled()) {
							m_logger.debug("Candidate permutation failed due to a conflict with a " + "fragment import; will try another if possible." + " (" + rethrow.getMessage() + ")");
						}
						return rethrow;
					}
				}
		...
		for (Entry<String, Blame> entry : pkgs.m_exportedPkgs.fast()) {
			...
			for (UsedBlames usedBlames : pkgBlames.values()) {
				if (!isCompatible(exportBlame, usedBlames.m_cap, resourcePkgMap)) {
					...
					rethrow = permuteUsedBlames(session, rethrow, allCandidates, resource, pkgName, null, usedBlames, permRef1, permRef2, mutated);
				}
			}
			...
			if (rethrow != null) {
				...
				if (m_logger.isDebugEnabled()) {
					m_logger.debug("Candidate permutation failed due to a conflict between " + "an export and import; will try another if possible." + " (" + rethrow.getMessage() + ")");
				}
				return rethrow;
			}
		}
		...
		for (Entry<String, List<Blame>> entry : allImportRequirePkgs.fast()) {
			...
			for (UsedBlames usedBlames : pkgBlames.values()) {
				...
				if (rethrow != null) {
					...
					if (m_logger.isDebugEnabled()) {
						m_logger.debug("Candidate permutation failed due to a conflict between " + "imports; will try another if possible." + " (" + rethrow.getMessage() + ")");
					}
					return rethrow;
				}
			}
		...
		for (Requirement req : resource.getRequirements(null)) {
			...
			if (cap != null) {
				if (!resource.equals(cap.getResource())) {
					rethrow = checkPackageSpaceConsistency(session, cap.getResource(), allCandidates, false, resourcePkgMap, resultCache);
					if (rethrow != null) {
		...
		for (Blame usedBlame : usedBlames.m_blames) {
			...
			if (rethrow == null) {
				if (requirementBlame == null) {
					...
					rethrow = new UseConstraintError(session.getContext(), allCandidates, resource, pkgName, requirementBlame, usedBlame);
				}
			}
	...
	private static OpenHashMap<String, Blame> calculateExportedPackages(ResolveSession session, Candidates allCandidates, Resource resource, OpenHashMap<String, Blame> exports, OpenHashMap<String, Blame> substitutes) {
		...
		if (wiring != null) {
			...
			if (session.getContext() instanceof FelixResolveContext) {
				substitutionWires = ((FelixResolveContext) session.getContext()).getSubstitutionWires(wiring);
			} else {
	...
	private static boolean isCompatible(Blame currentBlame, Capability candCap, Map<Resource, Packages> resourcePkgMap) {
	...
	private static boolean isCompatible(List<Blame> currentBlames, Capability candCap, Map<Resource, Packages> resourcePkgMap) {
	...
	private static Map<Resource, List<Wire>> populateWireMap(ResolveContext rc, Resource resource, Map<Resource, List<Wire>> wireMap, Candidates allCandidates) {
				...
				if ((cands != null) && (cands.size() > 0)) {
					for (Capability cand : cands) {
						...
						if (!cand.getNamespace().startsWith("osgi.wiring.") || !resource.equals(cand.getResource())) {
							...
							populateWireMap(rc, cand.getResource(), wireMap, allCandidates);
							...
							Wire wire = new WireImpl(unwrappedResource, getDeclaredRequirement(req), provider, getDeclaredCapability(cand));
							if (req.getNamespace().equals(PackageNamespace.PACKAGE_NAMESPACE)) {
					...
					for (Requirement req : fragment.getRequirements(null)) {
						...
						if (!isPayload(req)) {
							...
							if (req.getNamespace().equals(HostNamespace.HOST_NAMESPACE)) {
								fragmentWires.add(new WireImpl(getDeclaredResource(fragment), req, unwrappedResource, unwrappedResource.getCapabilities(HostNamespace.HOST_NAMESPACE).get(0)));
							}
							...
							else if (!rc.getWirings().containsKey(fragment) && !wireMap.containsKey(fragment)) {
	...
	private static Wire createWire(Requirement requirement, Candidates allCandidates) {
		...
		return new WireImpl(getDeclaredResource(requirement.getResource()), getDeclaredRequirement(requirement), getDeclaredResource(cand.getResource()), getDeclaredCapability(cand));
	}

	private static boolean isPayload(Requirement fragmentReq) {
		// this is where we would add other non-payload namespaces
		if (ExecutionEnvironmentNamespace.EXECUTION_ENVIRONMENT_NAMESPACE.equals(fragmentReq.getNamespace())) {
			return false;
		}
	...
	private static Map<Resource, List<Wire>> populateDynamicWireMap(ResolveContext rc, Resource resource, Requirement dynReq, Map<Resource, List<Wire>> wireMap, Candidates allCandidates) {
		...
		packageWires.add(new WireImpl(resource, dynReq, getDeclaredResource(dynCand.getResource()), getDeclaredCapability(dynCand)));
	...
	private static class Blame {
		...
		public String toString() {
			return m_cap.getResource() + "." + m_cap.getAttributes().get(PackageNamespace.PACKAGE_NAMESPACE) + (((m_reqs == null) || m_reqs.isEmpty()) ? " NO BLAME" : " BLAMED ON " + m_reqs);
		}
	...
	private static final class UseConstraintError extends ResolutionError {
		...
		public String getMessage() {
			if (m_blame2 == null) {
				return "Uses constraint violation. Unable to resolve resource " + Util.getSymbolicName(m_resource) + " [" + m_resource + "] because it exports package '" + m_pkgName + "' and is also exposed to it from resource " + Util.getSymbolicName(m_blame1.m_cap.getResource()) + " [" + m_blame1.m_cap.getResource() + "] via the following dependency chain:\n\n" + toStringBlame(m_blame1);
			} else {
				return "Uses constraint violation. Unable to resolve resource " + Util.getSymbolicName(m_resource) + " [" + m_resource + "] because it is exposed to package '" + m_pkgName + "' from resources " + Util.getSymbolicName(m_blame1.m_cap.getResource()) + " [" + m_blame1.m_cap.getResource() + "] and " + Util.getSymbolicName(m_blame2.m_cap.getResource()) + " [" + m_blame2.m_cap.getResource() + "] via two dependency chains.\n\nChain 1:\n" + toStringBlame(m_blame1) + "\n\nChain 2:\n" + toStringBlame(m_blame2);
			}
		}
				...
				for (int i = 0; i < blame.m_reqs.size(); i++) {
					...
					if ((i + 1) < blame.m_reqs.size()) {
						...
						if (cap.getNamespace().equals(PackageNamespace.PACKAGE_NAMESPACE)) {
							...
							sb.append("=");
							sb.append(cap.getAttributes().get(PackageNamespace.PACKAGE_NAMESPACE));
							Capability usedCap = getSatisfyingCapability(blame.m_reqs.get(i + 1));
							sb.append("; uses:=");
							sb.append(usedCap.getAttributes().get(PackageNamespace.PACKAGE_NAMESPACE));
						} else {
							sb.append(cap);
						}
						sb.append("\n");
						...
						if (export.getNamespace().equals(PackageNamespace.PACKAGE_NAMESPACE) && !export.getAttributes().get(PackageNamespace.PACKAGE_NAMESPACE).equals(blame.m_cap.getAttributes().get(PackageNamespace.PACKAGE_NAMESPACE))) {
							sb.append("; uses:=");
	...
	private static class EnhancedExecutor {
		...
		private final Deque<Future<Void>> awaiting = new ConcurrentLinkedDeque<>();
